...
export async function POST(request: Request) {
  try {
    ...
    if (!field) return fail(400, 'invalid_input', 'This question is not available.');
    const action =
      input.action === 'answer' && isSkipIntent(input.input ?? '') ? 'skip' : input.action;
    const validation =
      action === 'answer'
        ? validate(field, input.input ?? '', input.answers, input.signedBy)
        : { ok: true };
    if (!validation.ok)
      return Response.json(
        TurnResult.parse({
          validation: { ok: false, message: t(input.uiLanguage, validation.code!) },
          nextFieldId: field.id,
          question: `${t(input.uiLanguage, 'question')} ${field.label}`,
          done: false,
        }),
        { headers: privateHeaders },
      );
    let answers =
      action === 'answer' || action === 'skip'
        ? applyAnswer(
            input.schema,
            input.answers,
            field.id,
            action === 'skip' ? '' : (input.input ?? ''),
            action === 'skip' ? 'skipped' : 'answered',
          )
        : input.answers;
    if (action === 'answer')
      answers = attachSignature(input.schema, answers, field, input.signedBy);
    let next: string | null;
    try {
      next = nextField(input.schema, answers, field.id, action, input.targetFieldId);
    } catch {
      return fail(400, 'invalid_input', 'This question is not available.');
    }
    const nextItem = input.schema.fields.find((f) => f.id === next);
    const baseline: TurnResult = {
      validation: { ok: true },
      nextFieldId: next,
      question: nextItem
        ? `${t(input.uiLanguage, 'question')} ${nextItem.label}`
        : t(input.uiLanguage, 'done'),
      done: next === null,
      ...(action === 'explain'
        ? { explanation: field.help || t(input.uiLanguage, 'unknownHelp') }
        : {}),
    };
    const translate =
      !isDemoOnly() &&
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        try {
          if (!translate) send({ type: 'result', data: baseline });
          else send({ type: 'pending' });
          if (translate || translateHelp || translateQuestion) {
            const messages = [
              {
                role: 'user' as const,
                content: turnPrompt({
                  field,
                  action: translate ? 'translation' : translateQuestion ? 'question' : 'explain',
                  input: translate ? input.input : undefined,
                  uiLanguage: input.uiLanguage,
                  formLanguage: input.schema.language,
                }),
              },
            ];
            for await (const part of streamValidated(Wording, messages, request.signal)) {
              if ('partial' in part) {
                ...
                const normalized = part.result.normalizedValue;
                if (
                  translate &&
                  normalized !== undefined &&
                  !validate(field, normalized, answers).ok
                )
                  return fail(
                    502,
                    'llm_schema_fail',
                    'The translated value does not fit this field.',
                  );
                send({
                  type: 'result',
                  data: {
                    ...baseline,
                    question: part.result.question,
                    ...(translateQuestion ? { translated: true } : {}),
                    ...(translate
                      ? {
                          nextFieldId: field.id,
                          done: false,
                          validation: { ok: true, normalizedValue: normalized ?? input.input },
                        }
                      : { explanation: part.result.explanation || baseline.explanation }),
                  },
                });
              }
            }
    ...
    return new Response(stream, {
      headers: { ...privateHeaders, 'Content-Type': 'application/x-ndjson; charset=utf-8' },
    });
  } catch (error) {
