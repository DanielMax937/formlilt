...
export const Field = z.object({
  ...
  required: z.boolean(), options: z.array(z.string().min(1).max(150)).max(80).optional(),
  constraints: z.object({ pattern: z.string().max(200).optional(), min: z.number().finite().optional(), max: z.number().finite().optional(), maxLength: z.number().int().min(1).max(10000).optional(), dateFormat: z.enum(['MM/DD/YYYY','DD/MM/YYYY','YYYY-MM-DD']).optional() }).optional(),
  help: z.string().max(300).optional(),
  dependsOn: z.object({ fieldId: id, equals: z.string().max(150) }).optional(),
  exclusiveWith: z.array(id).max(20).optional(),
  anchor: z.object({ page: z.number().int().min(0).max(14), labelText: z.string().min(1).max(300).optional(), bbox: bounds.optional(), placement: z.enum(['right','below','inbox']) }).optional(),
...
export const FormSchemaBase = z.object({
  ...
  signature: z.object({ fieldId: id, dateFieldId: id.optional(), nameFieldId: id.optional() }).optional(),
...
export const FormSchema = FormSchemaBase.superRefine((form, ctx) => {
  ...
  if (new Set(form.sections.map(s => s.id)).size !== form.sections.length) fail('Section IDs must be unique');
  if (form.pages.some((p, i) => p.index !== i)) fail('Page indices must be contiguous and zero-based');
  const listed = form.sections.flatMap(s => s.fieldIds);
  if (listed.length !== fields.size || new Set(listed).size !== fields.size || listed.some(f => !fields.has(f))) fail('Each field must occur in exactly one section');
  for (const f of form.fields) {
    if (!form.sections.some(s => s.id === f.section && s.fieldIds.includes(f.id))) fail(`Invalid section for ${f.id}`);
    if (!f.anchor) fail(`Missing anchor for ${f.id}`);
    if (f.anchor && f.anchor.page >= form.pages.length) fail(`Page out of bounds for ${f.id}`);
    if (f.anchor && form.pages[f.anchor.page]?.kind === 'scan' && !f.anchor.bbox) fail(`Scan field ${f.id} requires a bounding box`);
    if (['select','multiselect'].includes(f.type) && !f.options?.length) fail(`Choice field ${f.id} requires options`);
    if (f.constraints?.min !== undefined && f.constraints.max !== undefined && f.constraints.min > f.constraints.max) fail(`Invalid numeric range for ${f.id}`);
    for (const ref of f.exclusiveWith ?? []) if (!fields.has(ref) || ref === f.id) fail(`Invalid exclusive reference for ${f.id}`);
    const seen = new Set([f.id]); let dep = f.dependsOn?.fieldId;
    while (dep) { if (seen.has(dep) || !fields.has(dep)) { fail(`Invalid or cyclic dependency for ${f.id}`); break; } seen.add(dep); dep = fields.get(dep)?.dependsOn?.fieldId; }
  }
  if (form.signature && fields.get(form.signature.fieldId)?.type !== 'signature') fail('Signature reference must name a signature field');
  if (form.signature?.dateFieldId && fields.get(form.signature.dateFieldId)?.type !== 'date') fail('Signature date reference must name a date field');
  if (form.signature?.nameFieldId && !fields.has(form.signature.nameFieldId)) fail('Signature name reference does not exist');
