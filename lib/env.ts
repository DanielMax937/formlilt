...
export const Env = z.object({
  ...
  LLM_TIMEOUT_MS: z.coerce.number().int().min(1000).max(300000).default(180000),
  LLM_STRUCTURED_OUTPUT: z.enum(['true','false']).default('false').transform(v => v === 'true'),
  ARK_BASE_URL: z.string().url().default('https://ark.cn-beijing.volces.com/api/v3'),
  ARK_API_KEY: optionalSecret, DOUBAO_MODEL: optionalSecret,
  OPENAI_BASE_URL: z.string().url().default('https://api.openai.com/v1'),
  OPENAI_API_KEY: optionalSecret, OPENAI_MODEL: z.string().default('gpt-6-astra'),
  UPSTASH_REDIS_REST_URL: z.string().optional().transform(v => v || undefined).pipe(z.string().url().optional()),
