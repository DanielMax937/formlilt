...
export const fail = (status: number, code: string, message: string): never => {
  throw { status, code, message } satisfies AppError;
};
    ...
    return Response.json(
      {
        error: {
          code: 'invalid_input',
          message: 'The form data is invalid. Please check it and try again.',
        },
      },
      { status: 400, headers: privateHeaders },
    );
  const parsed = z
    .object({ status: z.number().int().min(400).max(599), code: z.string(), message: z.string() })
    .safeParse(error);
  const result = parsed.success
    ? parsed.data
    : { status: 500, code: 'internal_error', message: 'Something went wrong. Please try again.' };
  return Response.json(
...
export async function limitedBody(request: Request, maxBytes: number) {
  if (Number(request.headers.get('content-length')) > maxBytes)
    fail(400, 'upload_rejected', 'This request is too large. Choose a smaller file.');
...
export async function readMultipart(request: Request) {
  if (!request.headers.get('content-type')?.startsWith('multipart/form-data'))
    fail(400, 'invalid_input', 'A file upload is required.');
  ...
  try {
    ...
    return fail(400, 'invalid_input', 'The upload could not be read.');
  }
}
export async function readJson(request: Request) {
  const bytes = await limitedBody(request, 750000);
  try {
...
export function sameOrigin(request: Request) {
  ...
  if (!permitted.includes(origin))
    fail(403, 'invalid_origin', 'Please use the form on this website.');
}
