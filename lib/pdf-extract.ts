...
export async function parseDocument(bytes: Uint8Array): Promise<ParsedDocument> {
  ...
  if (kind !== 'pdf') {
    if (kind === 'png') {
      ...
      if (size.width > 12000 || size.height > 12000 || size.width * size.height > 40000000)
        throw new Error('Image dimensions are too large.');
    }
    const pdf = await PDFDocument.create();
    const image = kind === 'jpg' ? await pdf.embedJpg(bytes) : await pdf.embedPng(bytes);
    if (image.width > 12000 || image.height > 12000 || image.width * image.height > 40000000)
      throw new Error('Image dimensions are too large.');
    return {
      source: 'image',
      pages: [
        { index: 0, widthPt: image.width, heightPt: image.height, kind: 'scan', textItems: [] },
      ],
      acroFields: [],
    };
  }
  ...
  try {
    ...
    for (let index = 0; index < proxy.numPages; index++) {
      ...
      if (textItems.length > 15000)
        throw new Error('This page has too much text to process safely.');
      pages.push({
        index,
        widthPt: view.width,
    ...
    for (const field of form.getFields()) {
      ...
      for (const widget of field.acroField.getWidgets()) {
        ...
        let pageIndex = doc
          .getPages()
          .findIndex((p) => pageRef && p.ref.toString() === pageRef.toString());
        if (pageIndex < 0)
        ...
        const view = page.getViewport({ scale: 1 });
        const corners = [
          ...view.convertToViewportPoint(rect.x, rect.y),
          ...view.convertToViewportPoint(rect.x + rect.width, rect.y + rect.height),
        ];
        const bbox: AcroField['bbox'] = [
          Math.min(corners[0], corners[2]) / view.width,
          Math.min(corners[1], corners[3]) / view.height,
          Math.max(corners[0], corners[2]) / view.width,
          Math.max(corners[1], corners[3]) / view.height,
        ];
        const options =
          'getOptions' in field && typeof field.getOptions === 'function'
            ? (field.getOptions() as string[])
            : undefined;
        acroFields.push({
          name: field.getName(),
