...
export function attachSignature(schema:FormSchema,answers:Answers,field:Field,signedBy?:string,date=today()):Answers{
 if(field.type!=='signature'||answers[field.id]?.status!=='answered'||!answers[field.id].value)return answers;
