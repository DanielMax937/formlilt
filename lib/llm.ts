...
export function getModel() {
  ...
  const config =
    env.LLM_PROVIDER === 'agent-im'
      ? {
          name: 'agent-im',
          baseURL: env.AGENT_IM_BASE_URL,
          apiKey: env.AGENT_IM_API_KEY,
          model: env.AGENT_IM_MODEL,
        }
  ...
  if (!config.apiKey || !config.model)
    return fail(503, 'llm_unconfigured', 'The form reader is not configured. Please try a demo.');
  return createOpenAICompatible({
    name: config.name,
    baseURL: config.baseURL,
    apiKey: config.apiKey,
    supportsStructuredOutputs: env.LLM_STRUCTURED_OUTPUT,
    ...(config.name === 'agent-im'
      ? {
...
export async function generateValidated<T>(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (/content.?filter|content.?policy|safety.?filter/i.test(message))
        return fail(
          422,
          'llm_blocked',
          'This document could not be processed. Try removing sensitive pages.',
        );
      ...
      throw {
        status: 502,
        code: text ? 'llm_schema_fail' : 'llm_error',
        message: 'The form reader could not finish. Please try again.',
        cause: error,
      };
    }
  }
...
export function normalizeAgentSchema(value: unknown): unknown {
  ...
  for (const [key, item] of Object.entries(input)) {
    ...
    if (key === 'items' && Array.isArray(item)) {
      output.prefixItems = item.map(normalizeAgentSchema);
      output.items = false;
...
export async function* streamValidated<T>(
  schema: z.ZodType<T>,
  messages: ModelMessage[],
  signal?: AbortSignal,
): AsyncGenerator<{ partial: unknown } | { result: T }> {
  const timeout = AbortSignal.timeout(getEnv().LLM_TIMEOUT_MS);
  const abortSignal = signal ? AbortSignal.any([signal, timeout]) : timeout;
  try {
    const output = streamObject({
      onError: () => {},
      model: getModel(),
      schema,
      mode: 'json',
    ...
    const feedback = NoObjectGeneratedError.isInstance(error)
      ? error.text?.slice(0, 6000)
      : undefined;
    const repairMessages: ModelMessage[] = feedback
      ? [
          ...messages,
          { role: 'assistant', content: feedback },
          { role: 'user', content: 'Repair this output to match the requested schema exactly.' },
        ]
      : messages;
