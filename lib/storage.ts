...
export async function createSession(schema:FormSchema, file:File, uiLanguage:UILanguage, demoSlug?:Session['demoSlug']) {
 ...
 await set('fillflow:file:'+session.id,{bytes:await file.arrayBuffer(),name:file.name,type:file.type,lastModified:file.lastModified});
 try{saveSession(session);}catch(error){await del('fillflow:file:'+session.id);throw error;}return session;
}
export async function getOriginal(id:string):Promise<File|undefined>{const stored:unknown=await get('fillflow:file:'+id);if(stored instanceof File)return stored;if(stored&&typeof stored==='object'&&'bytes' in stored&&stored.bytes instanceof ArrayBuffer&&'name' in stored&&typeof stored.name==='string'&&'type' in stored&&typeof stored.type==='string')return new File([stored.bytes],stored.name,{type:stored.type});return undefined;}
