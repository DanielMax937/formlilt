...
export async function renderFile(
  ...
  if (file.type === 'application/pdf' || /\.pdf$/i.test(file.name)) {
    ...
    try {
      ...
      for (let index = 1; index <= pdf.numPages; index++) {
        ...
        if (!ctx) throw new Error('Cannot render this page.');
  ...
  if (/\.(heic|heif)$/i.test(file.name) || /image\/hei[cf]/.test(file.type)) {
    ...
    throw new Error('Choose a PDF, JPG, PNG or HEIC image.');
  ...
  try {
    ...
    const ctx = canvas.getContext('2d');
    if (!ctx) throw new Error('Cannot read this image.');
    ctx.fillStyle = '#fff';
    ctx.fillRect(0, 0, size.width, size.height);
    ctx.drawImage(image, 0, 0, size.width, size.height);
    const original = new File([await jpeg(canvas)], file.name.replace(/\.[^.]+$/, '') + '.jpg', {
      type: 'image/jpeg',
    });
    onProgress(1, 1);
    return { original, pages: [original] };
  } finally {
...
export function usePageRender() {
  ...
  return {
    progress,
    render: (file: File) => renderFile(file, (page, total) => setProgress({ page, total })),
  };
}
