...
The public site has live uploads and answer translation enabled on Vercel. A production HTTP workflow passed extraction, translated-answer confirmation and verified PDF export; current-system-Chrome completion still awaits Mac unlock. Prepared uploads are limited to 4.4 MB, and basic rate limits apply per instance. Complex live extraction remains experimental: the reviewed school form now extracts 103 fields in about 102 seconds and exports correctly after source-grid placement repairs; this is one HTTP/PDF sample, not broad browser or latency acceptance. Do not claim that all forms are accurate, that extraction p95 is under 20 seconds, or that untested devices are verified. See `QUALITY.md` before publication.
