...
test('accepts a valid form and rejects duplicate/dangling field references', () => {
  ...
  expect(
    FormSchema.safeParse({ ...sample, fields: [...sample.fields, ...sample.fields] }).success,
  ).toBe(false);
  expect(
    FormSchema.safeParse({
      ...sample,
      sections: [{ id: 'personal', title: 'P', fieldIds: ['missing'] }],
    }).success,
  ).toBe(false);
});
test('rejects out-of-page anchors, inverted boxes, cyclic dependencies and missing options', () => {
  for (const change of [
    { anchor: { page: 1, placement: 'below' } },
    { anchor: { page: 0, placement: 'inbox', bbox: [0.8, 0.1, 0.2, 0.3] } },
    { dependsOn: { fieldId: 'name', equals: 'yes' } },
    { type: 'select', options: [] },
  ])
    expect(
      FormSchema.safeParse({ ...sample, fields: [{ ...sample.fields[0], ...change }] }).success,
    ).toBe(false);
});
