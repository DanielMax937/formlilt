...
for (const slug of ['change-of-address','insurance-claim','medical-release']) test(`extracts real ${slug} PDF text and widgets`, async () => {
  const result = await parseDocument(new Uint8Array(await readFile(`public/demo-forms/${slug}.pdf`)));
  expect(result.pages.length).toBeGreaterThanOrEqual(2); expect(result.pages.length).toBeLessThanOrEqual(4);
  ...
  console.info(slug, JSON.stringify({ pages: result.pages.map(p => ({ items: p.textItems.length,kind:p.kind })), acro: result.acroFields.length }));
});
test('photos use scan path; invalid bytes are rejected', async () => {
  ...
  expect(result.source).toBe('image'); expect(result.pages[0].kind).toBe('scan'); expect(result.pages[0].textItems).toEqual([]);
