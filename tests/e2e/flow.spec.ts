...
test('three answers, skip, back and jump preserve todo status', async ({ page }) => {
  ...
  for (const name of ['Rivera', 'Alex', 'J']) {
    await page.locator('#answer-input').fill(name);
  ...
  await expect(page.getByTestId('todo-f3')).toHaveAttribute('data-status', 'skipped');
  await page.getByRole('button', { name: 'Previous', exact: true }).click();
  await expect(page.getByTestId('todo-f3')).toHaveAttribute('data-status', 'current');
  await page.getByTestId('todo-f0').click();
  await expect(page.locator('#answer-input')).toHaveValue('Rivera');
  await page.reload();
  await expect(page.locator('#answer-input')).toHaveValue('Rivera');
});
