...
One-page production check (2026-09-17, Singapore): real Ark extraction **12.820s**, Chinese-to-English answer translation **4.623s**, and PDF export passed HTTP integration. All 10 native values and the rendered signature were verified; office fields stayed blank. This is one source, not a p95 estimate. See [production evidence](launch/ark-production-verification.json). The current system Chrome reached the live upload screen; full UI acceptance is waiting for the Mac to be unlocked. The flat-form follow-up returned 103 school fields from a real production upload in **101.807s** (`1a51973`). Visual review found table/duplicate-label placement errors, corrected in the current `9f7f388` deployment. A fresh HTTP export of that same extracted schema passed both-page visual review with 99 synthetic answers in **14.946s**. This export reused the prior live schema; it is not a new extraction or Chrome run. [Detailed evidence and preserved failures](launch/ark-object-extraction.json), [synthetic school PDF](launch/ark-school-filled.pdf).
