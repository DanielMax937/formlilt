...
Current deployment `346f652` restores the SPEC §5.5 date default: when the source gives no format, new extraction uses `MM/DD/YYYY`; explicit printed formats and saved sessions keep their existing format. A fresh current-Chrome Tinley upload completed all 10 questions, typed signature, review and download. Review and the native PDF field both show `09/17/2026`; all ten native values and blank office fields passed, with Poppler and Chrome visual review. The first extraction output required the existing JSON repair retry, which succeeded. This run does not establish the latency gate. [Date verification](launch/ark-date-default-verification.json) · [Synthetic PDF](launch/ark-date-default-filled.pdf).

The fallback-time fix (`a112a3c`) limits the two-page stage to 120 seconds or half the configured model budget, whichever is shorter. On the current 280-second budget this reserves about 160 seconds for the one whole-document attempt. This repairs the stalled-pair recovery path without changing the total deadline or guaranteeing faster successful extraction. Controlled tests cover both stages and caller/total cancellation. A fresh production school extraction returned 103 fields in **79.674s**, using the parallel path directly; this does not exercise real-model fallback recovery or satisfy the latency gate. After unlock, the current system Chrome successfully uploaded the school PDF and extracted 103 questions. Native entry of 35 synthetic answers (all 24 required fields valid), typed guardian signature, review edits and PDF download passed; both pages were visually checked in Poppler and Chrome. Optional/unvisited fields remain blank. This request also used the parallel path directly. [Budget verification](launch/ark-fallback-budget-verification.json) · [Chrome evidence](launch/ark-chrome-school-verification.json) · [Synthetic browser-exported PDF](launch/ark-chrome-school-filled.pdf).

Earlier production follow-up (`75cd1f8`, 2026-09-17): eligible two-page text PDFs now use two concurrent page requests with full document context and a bounded whole-document fallback. A fresh school upload returned **103 fields in 77.397s**, and the new schema exported with 99 synthetic answers in **21.426s**. Both PDF pages passed visual review. The runtime log confirms the parallel path. This single HTTP result improves on the earlier 101.807s sample but still fails the <20s target; it is not a p95 estimate. Parallel requests repeat input context and increase model input cost. A separate current-system-Chrome upload of the same school source timed out with HTTP 502; the page displayed its localized retry error and restored file controls. That failed attempt is preserved; the later `a112a3c` browser follow-up above passed. [Evidence including failure](launch/ark-page-pair-verification.json) · [Synthetic HTTP-exported PDF](launch/ark-page-pair-school-filled.pdf).

One-page production check (2026-09-17, Singapore): real Ark extraction **12.820s**, Chinese-to-English answer translation **4.623s**, and PDF export passed HTTP integration. All 10 native values and the rendered signature were verified; office fields stayed blank. This is one source, not a p95 estimate. See [production evidence](launch/ark-production-verification.json). Current system Chrome completed a real Tinley upload, all 10 questions, drawn signature, review and download. A translation timeout exposed a truncated-response bug, fixed in `dbf236a`; after reloading that deployment, Chinese 金色 → gold confirmation and a fresh export passed in the same saved session. See [native Chrome evidence](launch/ark-chrome-verification.json) and [synthetic PDF](launch/ark-chrome-filled.pdf). The flat-form follow-up returned 103 school fields from a real production upload in **101.807s** (`1a51973`). Visual review found table/duplicate-label placement errors, corrected by the `9f7f388` placement fix, also included in the current deployment. A fresh HTTP export of that same extracted schema passed both-page visual review with 99 synthetic answers in **14.946s**. This export reused the prior live schema; it is not a new extraction or Chrome run. [Detailed evidence and preserved failures](launch/ark-object-extraction.json), [synthetic school PDF](launch/ark-school-filled.pdf).
...
Latest local checks: **146 unit/API cases verified**, strict TypeScript and production build passed. The initial parallel suite had two PDF-test timeouts; a single-worker rerun of those files passed all 17 cases under the unchanged deadline. Doubao flat forms use named object rows with required source locators; native-widget forms retain the verified compact path. Repeated table columns align to source-drawn empty cells only when their counts match. Model repair uses nested validation paths; long-form signature and exclusive-choice references accept exact unique labels, reducing row-counting errors without relaxing source validation. See [Ark integration attempts](launch/ark-demo-verification.json) for current-provider successes and preserved failures. The extraction adapter preserves printed date formats, defaults unspecified dates to `MM/DD/YYYY` as required by SPEC §5.5, and carries mutually exclusive checkbox groups into answer validation. Split dates interrupted by printed words/year prefixes become separate blanks; non-native checkbox marks are centered. Extraction skips read-only, hidden and zero-area native widgets. The extraction prompt excludes office-only inputs, but model output still needs source review; optional office fields in the latest Ark run were skipped and stayed blank; role-specific signatures may remain optional for the appropriate person. Impossible dependency values are rejected; conditions such as age are shown as optional fields with source instructions instead of hiding them behind unsupported expressions. Scanned instructions can now appear as bounded quotations from the page image, including in mixed text/scan documents. The reviewed scan also preserves initials blanks separately from nearby Yes/No choices; see [scan-help evidence](launch/scan-help-verification.json). Five additional actual Chrome uploads (three Spanish/English library forms, a native Tinley dog-license form and a genuine paper scan) passed question entry, signature, review, download and visual export checks; see [extra-browser evidence](launch/extra-browser-verification.json). A German source also produced a separately reviewed library/API export, with ten checkbox conflict/allowed-answer checks in [extra-form evidence](launch/extra-form-results.json). These checks remain separate from the earlier six-case browser matrix.
