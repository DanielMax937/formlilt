...
The fallback-time fix (`a112a3c`) limits the two-page stage to 120 seconds or half the configured model budget, whichever is shorter. On the current 280-second budget this reserves about 160 seconds for the one whole-document attempt. This repairs the stalled-pair recovery path without changing the total deadline or guaranteeing faster successful extraction. Controlled tests cover both stages and caller/total cancellation. A fresh production school extraction returned 103 fields in **79.674s**, using the parallel path directly; this does not exercise real-model fallback recovery or satisfy the latency gate. Native Chrome follow-up is pending Mac unlock. [Verification](launch/ark-fallback-budget-verification.json).

Latest production follow-up (`75cd1f8`, 2026-09-17): eligible two-page text PDFs now use two concurrent page requests with full document context and a bounded whole-document fallback. A fresh school upload returned **103 fields in 77.397s**, and the new schema exported with 99 synthetic answers in **21.426s**. Both PDF pages passed visual review. The runtime log confirms the parallel path. This single HTTP result improves on the earlier 101.807s sample but still fails the <20s target; it is not a p95 estimate. Parallel requests repeat input context and increase model input cost. A separate current-system-Chrome upload of the same school source timed out with HTTP 502; the page displayed its localized retry error and restored file controls. This school browser flow remains unaccepted. [Evidence including failure](launch/ark-page-pair-verification.json) · [Synthetic HTTP-exported PDF](launch/ark-page-pair-school-filled.pdf).
